...
void findTandems(const string & seq, const string & tag)
{
  ...
  for (int i = 0; i < (int)seq.length(); i++)
  {
    ...
    for (int merlen = 1; merlen <= MAX_UNIT_LEN; merlen++)
    {
      ...
      if (j != merlen || (i+j+1 == (int)seq.length()))
      {
          ...
          if (((i-offset)/merlen >= MIN_REPORT_UNITS) && (i - offset >= MIN_REPORT_LEN))
          {
            ...
            { 
              ...
              else
              {
                ...
                for (int z = offset;       z < i+j;       z++) { cout << seq[z]; }
                for (int z = i+j;          z < i+j+FLANK; z++) { if (z < (int)seq.length()) { cout << (char) tolower(seq[z]); } }
  ...
  {
     // rest of genome seq
     for (int ppp = lastprinted; ppp < (int)seq.length(); ppp++) { fprintf(MUTATE_fp, "%c", seq[ppp]); }
     fprintf(MUTATE_fp, "\n");
  }
}
...
"\n"
" Usage: simple-repeat-finder -f fasta\n"
"\n"
...
"\n";
  ...
  while (!errflg && ((ch = getopt (argc, argv, "f:u:l:x:m:k:hF:M:L:X:S:b")) != EOF))
  {
    switch (ch)
    {
  ...
  while  (Fasta_Read (fp, s, tag))
  {
    ...
    for (int j = 0; j < (int)tag.length(); j++)
    {
      if (tag[j] == ' ')
      {
        tag[j] = '_';
