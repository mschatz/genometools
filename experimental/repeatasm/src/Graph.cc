...
void Graph_t::addSeqCov(int readid, const string & seq, int cov)
{
  // cerr << "AddSeqCov: " << seq << " " << cov << endl;
	...
	for (int offset = 0; offset < end; offset++)
	{
		...
		// cerr << readid << "\t" << offset << "\t" << uc << "\t" << vc << "\t" << cov << endl;
	    ...
	    if (offset == 0) { ui->second->cov_m += cov; }
		vi->second->cov_m += cov;
...
void Graph_t::trim(int readid, const string & seq, const string & qv, bool isRef)
{
    ...
    cerr << "Processed " << readid << " " << trim5 << " " << trim3 << endl;
}
...
void Graph_t::printDot(const string & filename)
{
	...
	for (mi = nodes_m.begin(); mi != nodes_m.end(); mi++)
	{
		...
		{
			fprintf(fp, "  %s [label=\"%d:%s | <F> %s... | <R> len=%d cov=%0.02f\" color=\"%s\"]\n",
				cur->nodeid_m.c_str(),
				nodes,
				cur->nodeid_m.c_str(),
...
{
	cerr << endl << "removing low coverage (<=" << TIP_COV_THRESHOLD << ")" << endl;
...
void Graph_t::removeTips()
{
	...
	do
	{
		...
		cerr << endl << "remove tips round: " << round << " <" << MAX_TIP_LEN << "bp" << endl;
...
void Graph_t::printStats()
{
	...
	for (gi = nodes_m.begin(); gi != nodes_m.end(); gi++)
	{
        ...
        int s = gi->second->strlen();
        span += s;
        if (s > maxspan) { maxspan = s; }
	}

	cerr << "stats: nodes: " << nodes_m.size() 
		<< " edges: " << edgecnt
		<< " total_span: " << span 
        << " max_span: " << maxspan 
        << " mean_span: " << ((float) span / nodes_m.size()) << endl;
}
